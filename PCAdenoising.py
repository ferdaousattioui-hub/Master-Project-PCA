...
# --- CONFIGURATION ESTHÉTIQUE DES GRAPHES (Style Jupyter Exact) ---
try:
...
# --- SECTION 2 : CAS PRATIQUE INTERACTIF (MNIST RESOLUTION) ---
    ...
    # Chargement et interpolation en 28x28 pour simuler MNIST d bsa7 bla crash
    digits = load_digits()
    
    # Simulation de la matrice 28x28 pour chaque image pour enlever tout effet flou
    X_raw_list = []
    for img in digits.images[:100]:
        # On fait un zoom haut de gamme sur la matrice pour avoir du 28x28 net
        resized = np.repeat(np.repeat(img, 3, axis=0), 3, axis=1) # Passe à 24x24
        padded = np.pad(resized, ((2,2), (2,2)), mode='constant') # Devient 28x28 exact !
        X_raw_list.append(padded.flatten())
        
    X_raw = np.array(X_raw_list) / 16.0  # Normalisation

    # Injection du bruit blanc gaussien (Exactement kima f Jupyter)
    np.random.seed(2026)
    ...
    col1.metric(label="📊 Espace d'origine", value="784 dimensions (pixels)")
    ...
    # --- VISUALISATION 1 : MATRICE EXPLICITE (SANS AUCUN FLOU) ---
    ...
    for i in range(n_digits):
        # Ligne 1 : Image Originale (MNIST Style)
    ...
    st.pyplot(fig2)
    st.success(f"✅ Analyse terminée ! La PCA a isolé les structures géométriques essentielles en format 28x28 HD.")
